...
A hydrology GeoTiff file was generated using JavaScript coding in GEE (see [Code.js](https://github.com/kathrynlhull/Spatial_Ecology_in_R_2025/blob/main/RExam_Sept7_26/Code.js)).  Significant tributary streams were added using the [MERIT Hydro Global Hydrography Dataset](https://developers.google.com/earth-engine/datasets/catalog/MERIT_Hydro_v1_0_1). 
...
# Conclusion
Time series satellite image analysis from 1985 to 2022 for the Waiparous Creek Basin indicates a clear visible and quantifiable signature from logging activities. Evidence of bare soil exposure is clearly apparent in the short-term as seen in False Colour image visualization immediately following logging activity. Spectral signatures of regenerating cutblocks persist more than 10 years post initial logging in 2010, including having less red light absorption possibly indicating more sparse vegetation cover. However, NDVI trend analysis was inconclusive given natural landscape and terrain complexity and coniferous forest canopies having inherent low NDVI values compared to broadleaf/deciduous vegetation. Another complication with the NDVI analysis is that source imagery composites were selected from a three month summer period (June to August), thus increasing potential for inter-annual variation in plant phenology due to climatic variation over the study period. A cursory terrain analysis confirms that most logged cutblocks have moderate to steep slope gradients, increasing potential for resulting sedimentation impacts into trout bearing streams. Logging as a land use thus represents a plausible hazard to water quality and habitat suitability for threatened Westslope Cutthroat Trout populations in the basin. Additional investigation is warranted to examine water quality and fish population and redd (gravel spawning 'nest') survey trends over this time period to quantify and verify a true hazard index. Logging impacts should also be considered in view of additive cumulative land use pressures in the watershed (e.g. energy and transportation infrastructure, motorized off-highway vehicles, livestock grazing and random camping). 
